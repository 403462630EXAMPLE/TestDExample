package com.hdy.swuperereshlayout;
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    private DefaultHeaderViewHolder defaultHeadVierHolder;
    private DefaultFooterViewHolder defaultFooterViewHolder;
    private OnPullRefreshListener mDefaultListener;
    private OnPullRefreshListener mListener;// 下拉刷新listener
    private OnPushLoadMoreListener mOnPushLoadMoreListener;// 上拉加载更多
    private OnPushLoadMoreListener mDefaultOnPushLoadMoreListener;// 上拉加载更多
    ...
    private void updateListenerCallBack() {
        ...
        if (mDefaultListener != null) {
            mDefaultListener.onPullDistance(distance);
        }
    ...
    private void setDefaultHeaderView() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.layout_head, null);
        defaultHeadVierHolder = new DefaultHeaderViewHolder(view);
        defaultHeadVierHolder.imageView.setVisibility(View.VISIBLE);
        defaultHeadVierHolder.progressBar.setVisibility(View.GONE);
        setHeaderView(view);
        setDefaultPullRefreshListener();
    }

    private void setDefaultFooterView() {
        View view = LayoutInflater.from(getContext()).inflate(R.layout.layout_footer, null);
        defaultFooterViewHolder = new DefaultFooterViewHolder(view);
        defaultFooterViewHolder.progressBar.setVisibility(View.GONE);
        defaultFooterViewHolder.imageView.setVisibility(View.VISIBLE);
        setFooterView(view);
        setDefaultPushLoadMore();
    }

    private void setDefaultPullRefreshListener() {
        mDefaultListener = new OnPullRefreshListener() {

            @Override
            public void onRefresh() {
                defaultHeadVierHolder.textView.setText("正在刷新...");
                defaultHeadVierHolder.imageView.setVisibility(View.GONE);
                defaultHeadVierHolder.progressBar.setVisibility(View.VISIBLE);
            }
            ...
            public void onPullEnable(boolean enable) {
                defaultHeadVierHolder.textView.setText(enable ? "松开刷新..." : "下拉刷新...");
                defaultHeadVierHolder.imageView.setVisibility(View.VISIBLE);
                defaultHeadVierHolder.progressBar.setVisibility(View.GONE);
                defaultHeadVierHolder.imageView.setRotation(enable ? 180 : 0);
            }
        };
    }

    private void setDefaultPushLoadMore() {
        mDefaultOnPushLoadMoreListener = new OnPushLoadMoreListener() {
            @Override
            public void onLoadMore() {
                defaultFooterViewHolder.textView.setText("正在加载...");
                defaultFooterViewHolder.imageView.setVisibility(View.GONE);
                defaultFooterViewHolder.progressBar.setVisibility(View.VISIBLE);
            }
            ...
            public void onPushEnable(boolean enable) {
                defaultFooterViewHolder.textView.setText(enable ? "松开加载..." : "上拉加载...");
                defaultFooterViewHolder.imageView.setVisibility(View.VISIBLE);
                defaultFooterViewHolder.progressBar.setVisibility(View.GONE);
                defaultFooterViewHolder.imageView.setRotation(enable ? 0 : 180);
            }
        };
    }
    ...
    public void setOnPushLoadMoreListener(
        ...
        if (mFooterViewContainer.getChildCount() < 1) {
...
//                startScaleDownAnimation(mRefreshListener);
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop < mTotalDragDistance) {
                        ...
                        if (mDefaultListener != null) {
                            mDefaultListener.onPullEnable(false);
                        }
                    } else {
                        if (mListener != null) {
                        ...
                        if (mDefaultListener != null) {
                            mDefaultListener.onPullEnable(true);
                        }
                    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mDefaultOnPushLoadMoreListener != null) {
                        mDefaultOnPushLoadMoreListener
                                .onPushEnable(pushDistance >= mFooterViewHeight);
                    }
                }
                break;
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (overscrollBottom < mFooterViewHeight
                        || (mOnPushLoadMoreListener == null && mDefaultOnPushLoadMoreListener == null)) {// 直接取消
                    pushDistance = 0;
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                    updateFooterViewPosition();
                    if (pushDistance == mFooterViewHeight && (mOnPushLoadMoreListener != null || mDefaultOnPushLoadMoreListener != null)) {
                        mLoadMore = true;
                        if (mOnPushLoadMoreListener != null) {
                            mOnPushLoadMoreListener.onLoadMore();
                        }
                        if (mDefaultOnPushLoadMoreListener != null) {
                            mDefaultOnPushLoadMoreListener.onLoadMore();
                        }
                    }
                } else {
    ...
    private void animatorFooterToBottom(int start, final int end) {
        ...
        valueAnimator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (end > 0 && (mOnPushLoadMoreListener != null || mDefaultOnPushLoadMoreListener != null)) {
                    // start loading more
                    mLoadMore = true;
                    if (mOnPushLoadMoreListener != null) {
                        mOnPushLoadMoreListener.onLoadMore();
    ...
    private void updatePushDistanceListener() {
        ...
        if (mDefaultOnPushLoadMoreListener != null) {
            mDefaultOnPushLoadMoreListener.onPushDistance(pushDistance);
        }
    }
    ...
    private static class DefaultFooterViewHolder{
        TextView textView;
        ImageView imageView;
        ProgressBar progressBar;

        public DefaultFooterViewHolder(View view) {
            textView = (TextView) view.findViewById(R.id.footer_text_view);
            imageView = (ImageView) view.findViewById(R.id.footer_image_view);
            progressBar = (ProgressBar) view.findViewById(R.id.footer_pb_view);
        }
    }
}
